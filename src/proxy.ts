...
const INTERNAL_API_ORIGIN =
  ...
  (process.env.NEXT_PUBLIC_API_URL ? process.env.NEXT_PUBLIC_API_URL.replace(/\/api\/?$/, '').replace('localhost', '127.0.0.1') : '') ||
  'http://127.0.0.1:8082';
...
export async function proxy(request: NextRequest) {
  ...
  if (pathname.startsWith('/api/')) {
    ...
    try {
      ...
      console.error(`[Proxy] Failed to rewrite to ${backendUrl}:`, err);
