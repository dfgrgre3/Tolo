...
interface StoredUserBehavior extends Omit<UserBehavior, 'lastUpdated' | 'timePattern' | 'navigationPattern'> {
  lastUpdated: string;
  timePattern: Omit<TimePattern, 'sessionTimes'> & { sessionTimes: string[] };
  navigationPattern: NavigationPattern; // NavigationPattern doesn't have Date fields
}
...
export class BehavioralAnalytics {
   ...
   */
  detectAnomalies(userId: string, currentContext?: AnalyticsContext): AnomalyDetection {
   ...
   */
  updateDevicePattern(userId: string, deviceInfo: DeviceInfo): void {
    const behavior = this.behaviors.get(userId);
    if (!behavior) return;
    ...
    try {
      ...
      if (stored) {
        ...
        this.behaviors = new Map(
          parsed.map((item: StoredUserBehavior) => [
            item.userId,
            {
              ...item,
