...
import type { LoginResponse } from './types/core.types';
...
export interface UnifiedAuthContextType {
  ...
  loginWithCredentials: (email: string, password: string, rememberMe?: boolean) => Promise<LoginResponse | TwoFactorRequiredResponse>;
...
export function UnifiedAuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(async (token: string, userData?: User, sessionId?: string) => {
    try {
      ...
      await authManager.login(token, userData, sessionId);
    } catch (err) {
