...
function resolveCategoryIcon(category: Category): LucideIcon {
  ...
  if (byId) return byId;
  if (category.icon?.includes("🧪") || category.icon?.includes("⚛")) return Beaker;
  if (category.icon?.includes("🌍")) return Globe;
  if (category.icon?.includes("🔢")) return Calculator;
  if (category.icon?.includes("💻") || category.icon?.includes("🖥")) return Code2;
  if (category.icon?.includes("🏛")) return Landmark;
  return BookOpen;
}
