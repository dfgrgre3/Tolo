...
export default function SchedulePage() {
	...
	useEffect(() => {
		const loadUser = async () => {
			...
			try {
				...
				if (!id) {
					const { data, error } = await safeFetch<{ id: string }>( "/api/users/guest", { method: "POST" }, { id: '' } );
					if (data && data.id) { id = data.id as string; safeSetItem(LOCAL_USER_KEY, id); }
				}
				if (id) setUserId(id);
			} catch (error) { logger.error("Failed to load user:", error); } 
	...
	function openLinked(b: PlanBlock) {
		...
		else if (b.examId) router.push(`/exams?focus=${b.examId}`);
	}
               ...
               {weekDates.map((day, dIdx) => (
                  ...
                  >
                     ...
                     <Button 
                        ...
                        variant="ghost" 
                        className="mt-auto h-12 w-full bg-white/5 border border-white/5 rounded-2xl gap-2 font-black text-xs uppercase hover:bg-white/10 hover:border-primary/30 transition-all group"
                     >
