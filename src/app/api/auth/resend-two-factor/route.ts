...
export async function POST(request: NextRequest) {
  return opsWrapper(request, async (req) => {
    ...
    logger.error('Resend two-factor code error:', error);
    return NextResponse.json(
...
async function updateLoginAttemptCode(loginAttemptId: string, code: string) {
  ...
  logger.info(`Updated login attempt ${loginAttemptId} with new code: ${code}`);
  return true;
}
...
async function sendTwoFactorCode(user: any, method: 'email' | 'sms' | 'app', code: string) {
  ...
  logger.info(`Sending 2FA code ${code} to ${user.email} via ${method}`);
