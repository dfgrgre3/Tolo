...
async function handleGetCourses(req: NextRequest, authenticatedUserId: string | null): Promise<NextResponse> {
  return opsWrapper(req, async (req) => {
    ...
    const cacheKey = `courses:list:p${page}:l${limit}:s${search}:c${category}:lv${level}:f${featured}:so${sort}`;
  ...
  return opsWrapper(request, async (req) => {
    return withAuth(req, async ({ userRole }) => {
      if (userRole !== "ADMIN" && userRole !== "TEACHER") {
        throw new AppError("You do not have permission to create a new course.", 403, ERROR_CODES.FORBIDDEN);
      }
      ...
      if (!name) {
        throw new AppError("Course name is required.", 400, ERROR_CODES.MISSING_PARAMETER);
      }
      ...
      if (duplicateSubject) {
        throw new AppError("A course with the same name, code, or slug already exists.", 409, ERROR_CODES.DUPLICATE_RESOURCE);
      }
