  ...
  return opsWrapper(request, async (req) => {
    try {
      ...
      if (!selectedProvider.apiKey) {
        return createErrorResponse(`„› «Õ API ·‹ ${selectedProvider.name} €Ì— „ÂÌ√`, 500);
      }
    ...
    88& 7?7?8 7?8y8 7?87?7?7?7?7? 8?8? JSON 8&7? 7?87?8 8y7? 7?87?7?88y7?:
    {
      ...
      if (selectedProvider === AI_PROVIDERS.OPENAI) {
        ...
        if (!response.ok) {
          ...
          return createErrorResponse("⁄–—«° ÌÊ«ÃÂ «·‰Ÿ«„ »⁄÷ «·’⁄Ê»«  «· ﬁ‰Ì… ›Ì  ﬁœÌ„ «·‰’«∆Õ Õ«·Ì«. Ì—ÃÏ «·„Õ«Ê·… „—… √Œ—Ï ·«Õﬁ«.", 500);
        }
      ...
      try {
        ...
        return successResponse({
          ...
          message: " „ ≈‰‘«¡ «·‰’«∆Õ Ê·ﬂ‰ ÕœÀ Œÿ√ ›Ì  ‰”ÌﬁÂ«. Ì—ÃÏ „—«Ã⁄… «·„Õ ÊÏ √œ‰«Â.",
          provider: selectedProvider.name
        });
      }
    } catch (error: unknown) {
