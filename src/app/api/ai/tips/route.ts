...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    - ط§ظ„ظ…ظˆط§ط¯ ط§ظ„ظ…ط³ط¬ظ„ط©: ${userData.subjectEnrollments?.map((s: any) => s.subject).join(", ") || "ظ„ط§ طھظˆط¬ط¯ ظ…ظˆط§ط¯ ظ…ط³ط¬ظ„ط©"}
    - ظ…طھظˆط³ط· ط¯ط±ط¬ط§طھ ط§ظ„ط§ظ…طھط­ط§ظ†ط§طھ ط§ظ„ط£ط®ظٹط±ط©: ${userData.examResults && userData.examResults.length > 0 ? (userData.examResults.reduce((sum: number, exam: any) => sum + exam.score, 0) / userData.examResults.length).toFixed(2) : "ظ„ط§ طھظˆط¬ط¯ ط¨ظٹط§ظ†ط§طھ"}
    - ظ…طھظˆط³ط· ظˆظ‚طھ ط§ظ„ط¯ط±ط§ط³ط© ط§ظ„ط£ط³ط¨ظˆط¹ظٹ: ${userData.progressSnapshots && userData.progressSnapshots.length > 0 ? (userData.progressSnapshots.reduce((sum: number, p: any) => sum + p.totalStudyMinutes, 0) / userData.progressSnapshots.length / 60).toFixed(2) + " ط³ط§ط¹ط§طھ" : "ظ„ط§ طھظˆط¬ط¯ ط¨ظٹط§ظ†ط§طھ"}
