  ...
  return (
    <div className="space-y-12 pb-20" dir="rtl">
      ...
      <motion.div 
        ...
        className={STYLES.glass + " p-8 md:p-12 flex flex-col lg:flex-row items-center justify-between gap-10 group"}
      >
        <div className="space-y-4 text-center lg:text-right flex-1">
          ...
          <p className="text-lg text-gray-400 font-medium max-w-2xl">
            أنت المتحكم في مصائر المحاربين. راقب الإحصائيات، وجّه الجيش الكان، وقم ببناء عظمة الجمهورية.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 bg-accent/10 p-4 rounded-[2rem] border border-border backdrop-blur-md">
          ...
          <AdminButton
            ...
            icon={Download}
            className="h-12 px-6 rounded-xl border-border text-foreground font-black uppercase text-[10px] tracking-widest hover:bg-accent"
          >
            تصدير المخطوطات
          </AdminButton>
          ...
          <AdminButton
            ...
            loading={refreshing}
            icon={RefreshCw}
            className="h-12 w-12 rounded-xl border-border text-foreground hover:bg-accent"
          />
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
         {quickActionsConfig.map((action, i) => (
           <motion.a
             ...
             className={STYLES.glass + " p-6 flex flex-col items-center justify-center gap-4 group hover:border-primary/50 hover:-translate-y-2 transition-all"}
           >
      ...
      {/* --- Intelligence Gathering (Search & Alerts) --- */}
      <AiCommandCenter />
         ...
         <div className="lg:col-span-3 space-y-8">
            <div className="flex flex-wrap items-center gap-6">
              <GlobalSearch 
                ...
                className="flex-1 min-w-[300px] h-16 rounded-2xl bg-card border border-border text-foreground font-bold"
              />
              <QuickFilters
                filters={[
                  { id: "today", label: "سجلات اليوم", icon: Compass, active: timeFilter === "today", onClick: () => setTimeFilter("today") },
                  { id: "week", label: "حملة الأسبوع", icon: Map, active: timeFilter === "week", onClick: () => setTimeFilter("week") },
                  { id: "month", label: "حصاد الشهر", icon: Calendar, active: timeFilter === "month", onClick: () => setTimeFilter("month") },
                ]}
              />
            </div>
            ...
            <SmartAlerts
              alerts={generateSmartAlerts({
                ...
                content: { subjects: data.stats.totalSubjects, exams: data.stats.totalExams, resources: 0 },
      ...
      {/* --- Advanced Tactical Maps (Secondary Charts) --- */}
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        ...
        <div className={STYLES.glass + " p-8 col-span-1 lg:col-span-2"}>
           ...
           <div className="h-[300px]">
              <ActivityHeatmap
                data={data.charts.activity.map((item) => ({
                  date: item.day,
                  count: item.sessions,
                }))}
                title="خريطة النشاط"
