    ...
    <div className="p-4 border-b flex items-center gap-3">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          {user.isOnline ? "متصل الآن" : user.lastSeen ? `آخر ظهور: ${formatTime(user.lastSeen)}` : "غير متصل"}
        </p>
      </div>
    </div>
  );
...
function MessageList({ messages, userId, loading, messagesEndRef }: { messages: Message[], userId: string | null, loading: boolean, messagesEndRef: React.RefObject<HTMLDivElement | null> }) {
  return (
...
function MessageInput({ newMessage, setNewMessage, handleSendMessage, sending }: { newMessage: string, setNewMessage: (msg: string) => void, handleSendMessage: (e: React.FormEvent) => void, sending: boolean }) {
  return (
...
function ConversationsList({ conversations, selectedUser, onSelect }: { conversations: Conversation[], selectedUser: User | null, onSelect: (c: Conversation) => void }) {
  return (
