...
const ProgressControl = memo(
  ...
  }: {videoRef: React.RefObject<HTMLVideoElement | null>;duration: number;buffered: number;onSeek: (val: number) => void;}) => {
...
const VolumeControl = memo(
  ...
  }: {volume: number;isMuted: boolean;onToggleMute: () => void;onVolumeChange: (val: number) => void;}) =>
...
export function CourseVideoPlayer({
  ...
  const youtubeEmbedUrl = useMemo(
    ...
    `https://www.youtube.com/embed/${parseYouTubeId(videoUrl)}?rel=0&modestbranding=1&autoplay=0&playsinline=1&controls=1` :
    videoUrl,
    [videoUrl]
  );
  ...
  const _showToast = useCallback((msg: string) => {
    setToastMessage(msg);
  ...
  const syncProgressToServer = useCallback(
    (currentTime: number) => {
      if (provider !== "html5" || !Number.isFinite(currentTime)) return;
  ...
  const saveProgress = useCallback(
    (force = false) => {
      ...
      syncProgressToServer(video.currentTime);
    },
    [provider, storageKey, syncProgressToServer]
  );
  ...
  const toggleFullscreen = useCallback(async () => {
    ...
    if (!element) return;
    try {
      if (document.fullscreenElement) await document.exitFullscreen();else
  ...
  const _togglePictureInPicture = useCallback(async () => {
    ...
    try {
      ...
      // Ignore unsupported picture-in-picture requests.
    }
    resetControlsTimeout();
  }, [resetControlsTimeout]);
  ...
  useEffect(() => {
    ...
    const updateTime = () => {
      ...
      if (
      video.duration > 0 &&
      video.currentTime / video.duration * 100 >= AUTO_COMPLETE_PERCENT)
  ...
  const onLoadedMetadata = useCallback(() => {
    ...
    setIsLoading(false);
    let localProgress: StoredVideoProgress | null = null;

    try {
      const saved = localStorage.getItem(storageKey);
      if (saved) {
        localProgress = JSON.parse(saved) as StoredVideoProgress;
      }
    } catch {
      localProgress = null;
    }
    ...
    void (async () => {
      let resumeCandidate = localProgress?.currentTime ?? null;
      ...
      try {
        ...
        if (response.ok) {
          ...
          const serverProgress = (result.data ?? result) as ServerLessonProgress;
          const serverPosition =
          typeof serverProgress.lastVideoPosition === "number" ?
          serverProgress.lastVideoPosition :
          null;
          ...
          if (serverPosition !== null && serverPosition > 0) {
            if (resumeCandidate === null || serverUpdatedAt >= localUpdatedAt) {
              resumeCandidate = serverPosition;
            }
          }
        }
      } catch {
        // Ignore resume sync failures and fall back to local storage.
      }

      if (
      resumeCandidate !== null &&
      resumeCandidate > 5 &&
      resumeCandidate < video.duration - 5)

      setResumeTime(resumeCandidate);
    })();
  }, [lessonId, storageKey]);
  ...
  if (provider === "youtube") {
    return (
      <div
        className={cn(
          "group relative w-full overflow-hidden rounded-2xl border border-white/10 bg-black shadow-2xl",
          className
        )}>
        ...
        <div className="flex justify-between bg-slate-900 px-5 py-3 text-xs text-slate-200">
          ...
          <button
            ...
            className="rounded-full bg-emerald-500/10 px-3 py-1.5 font-bold text-emerald-400 border border-emerald-500/20">
  ...
  return (
    <div
      ...
      className={cn(
        "relative aspect-video w-full overflow-hidden rounded-2xl bg-black shadow-2xl ring-1 ring-white/10 transition-all group/player",
      ...
      <AnimatePresence>
        ...
        <div className="absolute bottom-24 right-6 left-6 z-50 flex items-center justify-between rounded-xl border border-blue-500/30 bg-blue-950/80 p-4 text-white backdrop-blur-xl md:w-80 md:mx-auto">
            ...
            className="rounded-lg bg-blue-500 px-4 py-1.5 text-xs font-bold">
      ...
      <div
        className={cn(
          "absolute inset-x-0 bottom-0 flex flex-col justify-end bg-gradient-to-t from-black/90 p-4 transition-opacity duration-300",
        ...
        <AnimatePresence>
          ...
          <motion.div
            ...
            exit={{ opacity: 0, y: 10 }}
            className="absolute bottom-20 left-4 w-40 rounded-xl bg-black/90 p-2 border border-white/10">
            ...
            <button
              ...
              className={cn(
                "w-full px-4 py-2 text-left text-sm text-white hover:bg-white/10 rounded-lg",
                playbackRate === rate && "text-blue-400 font-bold"
              )}>
