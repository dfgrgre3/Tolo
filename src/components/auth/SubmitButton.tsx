...
import { triggerHapticFeedback, buttonVariants } from '@/app/(auth)/_utils/auth-animations';
...
export const SubmitButton = memo<SubmitButtonProps>(({
  ...
  const computedAriaLabel = useMemo(() => {
    ...
    if (isLoading) return loadingText || 'جارٍ المعالجة...';
    return typeof children === 'string' ? children : 'إرسال';
  }, [ariaLabel, isLoading, isSuccess, loadingText, children]);
    ...
    if (isLoading) {
      return (
        <span className="flex items-center justify-center gap-2">
          ...
          {loadingText !== undefined ? loadingText : 'جارٍ تسجيل الدخول...'}
        </span>
      );
    }
    return children;
  }, [isLoading, isSuccess, children, loadingText]);
  return (
    <motion.button
