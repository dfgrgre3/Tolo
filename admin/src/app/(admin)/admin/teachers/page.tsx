...
import { Users, UserPlus, GraduationCap, BookOpen, Star, Trash2, Edit } from "lucide-react";
    ...
    <div className="space-y-10 pb-20" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <AdminStatsCard
          ...
          value={data?.data?.teachers?.filter(t => t.rating > 0).length ?? 0}
          icon={Users}
          color="green"
          description="لديهم تقييم"
        />
        <AdminStatsCard
          title="متوسط التقييم"
          value={
            data?.data?.teachers?.length
              ? (data.data.teachers.reduce((sum, t) => sum + (t.rating || 0), 0) / data.data.teachers.length).toFixed(1)
              : "0.0"
          }
          icon={Star}
          color="yellow"
