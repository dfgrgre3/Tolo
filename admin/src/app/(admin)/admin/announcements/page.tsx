...
export default function AdminAnnouncementsPage() {
  ...
  const handleResendAsNotification = React.useCallback(async (announcement: Announcement) => {
    const toastId = toast.loading("جاري إرسال الإشعار لجميع المستخدمين...");
    try {
      // Fetch all user IDs (up to 1000 in one call for broadcast)
      const usersRes = await adminFetch(`${apiRoutes.admin.users}?limit=1000&fields=id`);
      if (!usersRes.ok) throw new Error("فشل في جلب قائمة المستخدمين");
      const usersJson = await usersRes.json();
      const users: { id: string }[] = usersJson?.data?.users || usersJson?.users || [];
      if (users.length === 0) {
        toast.error("لا يوجد مستخدمون لإرسال الإشعار إليهم", { id: toastId });
        return;
      }
      ...
      const notifType = typeMap[announcement.type] || "info";

      const broadcastRes = await adminFetch("/api/admin/notifications/broadcast", {
        method: "POST",
      ...
      if (!broadcastRes.ok) {
        ...
        throw new Error((err as any)?.error || "فشل في إرسال البث");
      }

      const result = await broadcastRes.json();
      toast.success(
        `تم إرسال الإشعار بنجاح إلى ${result?.summary?.success ?? userIds.length} مستخدم`,
        { id: toastId }
      );

      logAdminAction("CREATE", "notification_broadcast", {
        entityId: announcement.id,
        entityName: announcement.title,
        details: { totalUsers: userIds.length, broadcastId: result?.broadcastId },
      });
    } catch (err: unknown) {
      toast.error(err instanceof Error ? err.message : "خطأ في إرسال الإشعار", { id: toastId });
    }
  }, []);
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <RowActions
          ...
          extraActions={[
            { icon: Send, label: "إعادة إرسال كإشعار", onClick: (a) => handleResendAsNotification(a) },
          ]}
        />
      ),
    },
