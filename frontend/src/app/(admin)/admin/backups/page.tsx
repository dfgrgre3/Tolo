...
export default function AdminBackupsPage() {
  ...
  const { data: queryData, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (Array.isArray(data) ? data : []) as Backup[];
    },
  ...
  const backups = React.useMemo(() => {
    ...
    if (typeof queryData === "object") {
      ...
      if (rawData && typeof rawData === "object" && Array.isArray(rawData.backups)) {
        return rawData.backups as Backup[];
      }
      if (Array.isArray(rawData)) {
        return rawData as Backup[];
      }
    }
    return [];
  }, [queryData]);
