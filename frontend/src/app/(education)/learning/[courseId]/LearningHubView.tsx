...
import { useLearningHub, type LearningHubOptions } from "./hooks/useLearningHub";
...
// The hub UI is shared by two routes: the legacy /learning/[courseId] URL and
// the canonical /courses/[slug]/learn/[lessonId] one. Both mount this view; only
// the way the course and the initial lesson are resolved differs.
export function LearningHubView(options: LearningHubOptions) {
  const router = useRouter();
  const hub = useLearningHub(options);
              ...
              <section className="rounded-[32px] border border-emerald-500/20 bg-emerald-500/10 p-6">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      router.push(`/courses/${options.courseSlug ?? hub.courseId}`)
                    }
                  >
                    العودة إلى الدورة
                  </Button>
