      ...
      filteredConversations.map((conv) => {
       ...
       return (
        <button
         ...
         className={`w-full flex gap-3 p-3 rounded-xl text-right items-start ${
      ...
      <div className="px-4 py-2 bg-slate-50/50 dark:bg-slate-900/30 border-t border-slate-100 dark:border-slate-850 flex items-center gap-2 overflow-x-auto no-scrollbar">
       ...
       {quickReplies.map((reply, i) => (
        <button
         ...
         className="px-2.5 py-1 bg-card hover:bg-slate-100 dark:hover:bg-slate-800 text-[9px] font-medium text-slate-600 dark:text-slate-300 rounded-full border border-slate-200 dark:border-slate-800 whitespace-nowrap "
        >
         {reply}
        </button>
       ))}
