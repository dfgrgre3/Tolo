...
export default function WeeklySchedule({
  ...
  const saveSchedule = useCallback(async () => {
    ...
    try {
      ...
      // Legacy schedules may carry a numeric version; current writes use '2.0'.
      // Read through an unknown-typed view instead of `any` so a shape
      // change fails loudly here rather than posting garbage upstream.
      const scheduleVersion = (schedule as unknown as { version?: unknown } | null)?.version;
      const savedSchedule = await saveScheduleRaw<Schedule>({
        version: typeof scheduleVersion === 'number' ? scheduleVersion : undefined,
