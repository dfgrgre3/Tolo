...
export const ExamsModal = memo(({ subject, onClose }: ExamsModalProps) => {
  ...
  return (
    <div
      className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 fade-in"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    >
      <div
        className="bg-[#0f172a] border border-white/10 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col zoom-in-95"
      ...
      >
        ...
        <div className="bg-[#1e293b]/50 p-6 border-b border-white/10 flex justify-between items-center shrink-0">
          ...
          <button
            onClick={onClose}
            className="text-gray-400 hover:text-white hover:bg-white/10 p-2 rounded-full"
            aria-label="إغلاق"
          >
        ...
        <div className="overflow-y-auto p-6 space-y-4 custom-scrollbar">
          {subject.exams.length > 0 ? (
            subject.exams.map((exam) => (
              <div key={exam.id} className="group relative overflow-hidden rounded-xl border border-white/5 bg-white/5 p-4 hover:border-primary/30 hover:bg-white/10 hover:shadow-lg hover:shadow-primary/5">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative z-10">
                  <div>
                    <h4 className="font-bold text-lg text-gray-100 group-hover:text-primary flex items-center gap-2">
                      {exam.title}
                  ...
                  <Link
                    href={`/exams/${exam.id}`}
                    className="bg-primary hover:bg-primary/90 text-primary-foreground px-6 py-2.5 rounded-lg font-bold shadow-[0_0_15px_rgba(124,58,237,0.3)] hover:shadow-[0_0_25px_rgba(124,58,237,0.5)] w-full sm:w-auto text-center flex items-center justify-center gap-2"
                  >
