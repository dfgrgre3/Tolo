import {
  fetchCategoriesRaw,
  fetchHomeBlogFeedRaw,
  fetchHomepageStatsRaw,
  fetchPublicCoursesRaw,
  fetchTeachersRaw,
...
export async function fetchCategories(): Promise<Category[]> {
  try {
    const data = await fetchCategoriesRaw("?limit=12");
...
export async function fetchCourses(sort: CourseSort): Promise<CourseItem[]> {
  ...
  const url = `?isPublished=true&isActive=true&limit=8&sort=${SORT_FIELDS[sort]}&order=desc`;
  try {
    const data = await fetchPublicCoursesRaw<ApiSubjectsResponse>(url);
    return data.items || [];
  } catch {
    return [];
  }
...
export async function fetchInstructors(): Promise<Instructor[]> {
  try {
    const data = await fetchTeachersRaw<Instructor[]>("?limit=6");
...
export async function fetchBlogPosts(): Promise<BlogPost[]> {
  try {
    const data = await fetchHomeBlogFeedRaw<{ posts: BlogPost[] }>('?limit=4&published=true');
    return data.posts;
  } catch {
    return [];
  }
...
export async function fetchStats(): Promise<PlatformStats | null> {
  try {
    const data = await fetchHomepageStatsRaw<HomepageResponse>();
