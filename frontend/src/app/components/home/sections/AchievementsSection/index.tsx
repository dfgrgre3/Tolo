...
function validateAchievements(data: unknown): ApiAchievement[] {
  if (!data || typeof data !== "object" || !("achievements" in data)) return [];
  const list = (data as { achievements: unknown }).achievements;
...
function validateProgress(data: unknown): ApiProgress {
  ...
  return {
    totalXP: typeof (data as ApiProgress).totalXP === "number" ? (data as ApiProgress).totalXP : 0,
    level: typeof (data as ApiProgress).level === "number" ? (data as ApiProgress).level : 1,
  };
}
...
export const AchievementsSection = memo(function AchievementsSection({
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (!isAuthenticated || !user?.id || authLoading) {
        logger.warn("AchievementsSection: Auth not ready, skipping fetch");
        setLoading(false);
        return;
      }
  ...
  if (loading) {
    return (
      <div 
        className="h-64 animate-pulse bg-white/5 rounded-3xl flex items-center justify-center"
        role="status"
        aria-busy="true"
        ...
        <div className="text-center">
          <Link href="/settings/achievements">
            <Button size="lg" variant="ghost" className="group text-gray-300 hover:text-white hover:bg-white/10">
              ...
              <ArrowRight 
                className="mr-2 h-4 w-4 group-hover:translate-x-1 transition-transform rtl:rotate-180 rtl:group-hover:-translate-x-1" 
                aria-hidden="true"
              />
            </Button>
          </Link>
