...
export function InteractiveQuestionOverlay({
  ...
  const handleSubmit = () => {
    ...
    // Only allow proceeding if answer is correct
    if (correct) {
      setCanProceed(true);
    }
  };
      ...
      >
        ...
        <div className="bg-white/5 p-6 flex justify-end gap-3">
          {!isSubmitted ? (
            ...
            <Button
              disabled
              className="rounded-2xl bg-slate-600 px-8 text-slate-300 h-12 font-bold cursor-not-allowed"
            >
              <AlertCircle className="mr-2 h-4 w-4" />
              إجابة خاطئة - يرجى المحاولة مرة أخرى
            </Button>
          )}
        </div>
      </m.div>
