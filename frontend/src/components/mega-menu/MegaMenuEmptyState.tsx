...
import { Search, Sparkles, BookOpen, Brain, Calendar } from "lucide-react";
...
export function MegaMenuEmptyState({ searchQuery, onClose }: MegaMenuEmptyStateProps) {
	return (
		<div className="flex flex-col items-center justify-center py-16 px-4 max-w-xl mx-auto">
			<div className="mb-6 p-5 rounded-2xl bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border border-primary/20 shadow-lg shadow-primary/5 relative">
					...
					{QuickActions.map((action) => {
						...
						return (
							<Link
								...
								href={action.href}
								onClick={onClose}
								className={`flex items-center gap-3 p-3 rounded-xl bg-gradient-to-r border hover:shadow-md hover:shadow-primary/5 ${action.color}`}
							>
