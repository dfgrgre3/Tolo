...
export function HeaderMobileMenuEnhanced({
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/60 dark:bg-black/80 z-[60] lg:hidden backdrop-blur-sm" onClick={closeMobileMenu} />
      <div
        ref={mobileMenuRef}
      ...
      >
        <div className="flex items-center justify-between p-4 pb-2 border-b border-border/20">
          ...
          <Button variant="ghost" size="icon" onClick={closeMobileMenu} className="rounded-full hover:bg-destructive/10 hover:text-destructive h-9 w-9">
            <X className="h-4 w-4" />
          </Button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-thumb-primary/10 hover:scrollbar-thumb-primary/20 scrollbar-track-transparent -webkit-overflow-scrolling: touch">
          <div className="px-4 py-3">
            <form onSubmit={handleSearch} className="relative group">
              <div className="absolute inset-0 bg-primary/5 rounded-2xl -m-1 opacity-0 pointer-events-none" />
              <Input
                type="text"
                ...
                className={cn(
                  "w-full ps-10 pe-3 h-11 rounded-2xl bg-muted/50 border-transparent focus:bg-background text-start text-base",
                  isSearchFocused && "ring-2 ring-primary/20 border-primary/20"
                )}
              />
              <Search className={cn("absolute start-3 top-1/2 -translate-y-1/2 h-4.5 w-4.5 text-muted-foreground", isSearchFocused && "text-primary")} />
            </form>
          </div>
          ...
          <div className="px-4 py-2">
            {isLoading ? (
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <Button variant="outline" className="rounded-2xl h-10 border-primary/20 hover:bg-primary/5 text-primary gap-1.5 font-bold text-sm" asChild>
                  <Link href={loginUrl} onClick={closeMobileMenu}>
                    <LogIn className="h-3.5 w-3.5" />
                    دخول
                  </Link>
                </Button>
                <Button className="rounded-2xl h-10 bg-primary hover:bg-primary/90 text-primary-foreground gap-1.5 shadow-lg shadow-primary/20 font-bold text-sm" asChild>
                      ...
                      {hasMegaMenu ? (
                        <div className="space-y-1">
                          <button
                            ...
                            className={cn(
                              "w-full flex items-center justify-between gap-2.5 p-3 rounded-xl border border-transparent touch-manipulation",
                              active ? "bg-primary/10 text-primary font-bold border-primary/10" : "hover:bg-muted font-medium text-foreground/80 hover:text-foreground"
                            )}
                          >
                            <div className="flex items-center gap-2.5">
                              <div className={cn("flex items-center justify-center w-7 h-7 rounded-lg", active ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground")}>
                            ...
                            <ChevronDown className={cn("h-3.5 w-3.5 transition-transform", expandedMenus.has(item.href) ? "rotate-180 text-primary" : "text-muted-foreground")} />
                          </button>
                                  ...
                                  {category.items.map((subItem) => {
                                    ...
                                    return (
                                      <Link
                                        ...
                                        onClick={closeMobileMenu}
                                        className={cn(
                                          "flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-sm touch-manipulation",
