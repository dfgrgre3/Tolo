...
export function HeaderNavLink({
  ...
  return (
    ...
    >
      {variant === "mega" && <m.div className={navLayoutStyles.megaItemGlow} initial={false} />}
      {variant === "mega" && active && (
        <m.div className={navLayoutStyles.megaItemActiveBar} layoutId="activeMegaMenuItem" transition={{ type: "spring", stiffness: 380, damping: 30 }} />
      )}
      ...
      <div className="flex-1 min-w-0 z-10">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {badge && <m.span initial={variant === "mega" ? { scale: 0, rotate: -180 } : false} animate={variant === "mega" ? { scale: 1, rotate: 0 } : undefined} transition={variant === "mega" ? { type: "spring", stiffness: 200, damping: 15 } : undefined} className={cn(navBadgeStyles({ variant, compact: isCompact }))}>{badge}</m.span>}
        </div>
