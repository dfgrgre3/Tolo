...
export function contractLogin(payload: ContractLoginRequest) {
  return client.POST('/api/v1/auth/login', { body: payload }).then((result) => ({
    ...result,
    data: result.data == null ? result.data : unwrapOpenApiPayload(result.data),
  }));
}

export function contractVerifyMfa(payload: { challengeId: string; code: string; rememberMe?: boolean }) {
  return client.POST('/api/v1/auth/mfa/verify', { body: payload }).then((result) => ({
    ...result,
    data: result.data == null ? result.data : unwrapOpenApiPayload(result.data),
  }));
}
