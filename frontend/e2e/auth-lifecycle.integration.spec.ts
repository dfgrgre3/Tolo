...
test.describe('real auth lifecycle (PostgreSQL + Redis + Mailpit)', () => {
  ...
  test('registers, verifies email, enrolls MFA, resets password and schedules deletion', async ({ request }) => {
    ...
    const registration = await request.post('/api/v1/auth/register', { headers, data: {
    ...
    const loginBeforeVerification = await request.post('/api/v1/auth/login', { headers, data: { email, password } });
    expect(loginBeforeVerification.ok()).toBeTruthy();

    const verify = await request.post('/api/v1/auth/verify-email', { headers, data: { code: verificationCode } });
    expect(verify.ok()).toBeTruthy();

    const mfaSetup = await request.post('/api/v1/auth/mfa/setup', { headers });
    ...
    const mfaEnable = await request.post('/api/v1/auth/mfa/enable', { headers, data: { code: totp(secret) } });
    expect(mfaEnable.ok()).toBeTruthy();

    const forgot = await request.post('/api/v1/auth/forgot-password', { headers, data: { email } });
    ...
    const resetVerify = await request.post('/api/v1/auth/forgot-password/verify-code', { headers, data: { email, code: resetCode } });
    expect(resetVerify.ok()).toBeTruthy();
    const reset = await request.post('/api/v1/auth/reset-password', { headers, data: { newPassword: 'E2ePassword!456' } });
    expect(reset.ok()).toBeTruthy();

    const loginAfterReset = await request.post('/api/v1/auth/login', { headers, data: { email, password: 'E2ePassword!456', rememberMe: false } });
    expect(loginAfterReset.ok()).toBeTruthy();
    const recentAuth = await request.post('/api/v1/auth/reauthenticate', { headers, data: { password: 'E2ePassword!456' } });
    expect(recentAuth.ok()).toBeTruthy();
    const deletion = await request.delete('/api/v1/auth/account', { headers, data: { password: 'E2ePassword!456', reason: 'e2e cleanup' } });
